...
def _auto_fix_file(filepath: str) -> list[str]:
    ...
    try:
        ...
        sys.path.insert(0, os.path.join(PROJECT_DIR, "scripts", "manual-publisher"))
        import validator
        import transformer
...
def build_and_deploy(count: int) -> bool:
    ...
    if not errors:
        ...
        msg = (
            f"🚨 자동 발행 실패 — 수동 발행 필요\n"
            f"원인: 알 수 없는 빌드 에러\n"
            f"실행 명령: python3 scripts/manual-publisher/publisher.py\n"
    ...
    if not fixed_any:
        msg = (
            ...
            f"에러 ({len(errors)}건):\n{error_summary}\n"
            f"\n수동 실행: python3 scripts/manual-publisher/publisher.py"
        )
        send_telegram(msg)
        return False
...
def _deploy(count: int) -> bool:
    ...
    if result.returncode != 0:
        msg = (
            ...
            f"stderr: {result.stderr[-300:]}\n"
            f"수동 실행: python3 scripts/manual-publisher/publisher.py"
        )
        print(msg)
        send_telegram(msg)
        return False
